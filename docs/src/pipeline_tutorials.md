...
The workflow contains two jobs. The first job is the `generate` job, which generates a list of package names to be tested as an integration test. The `run-matrix` job is a job template and takes a package name and runs an integration test job for the specific package name. Therefore, `run-matrix` is executed N times.
